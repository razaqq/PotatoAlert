...
[[maybe_unused]] bool Utilities::setBlurEffectEnabled(const QWindow *window, const bool enabled, const QColor &gradientColor)
{
	Q_ASSERT(window);
	if (!window)
		return false;
...
[[maybe_unused]] bool Utilities::isLightThemeEnabled()
{
...
[[maybe_unused]] bool Utilities::isHighContrastModeEnabled()
{
	HIGHCONTRASTW hc;
...
[[maybe_unused]] bool Utilities::isDarkFrameEnabled(const QWindow *window)
{
	Q_ASSERT(window);
	if (!window)
		return false;
...
[[maybe_unused]] bool Utilities::isTransparencyEffectEnabled()
{
	if (!isWin10OrGreater())
		return false;
	bool ok = false;
...
[[maybe_unused]] QImage Utilities::getDesktopWallpaperImage(const int screen)
{
	if (isWin8OrGreater())
	{
...
[[maybe_unused]] QColor Utilities::getDesktopBackgroundColor(const int screen)
{
	Q_UNUSED(screen)
	if (isWin8OrGreater())
	{
...
[[maybe_unused]] Utilities::DesktopWallpaperAspectStyle Utilities::getDesktopWallpaperAspectStyle(const int screen)
{
	Q_UNUSED(screen)
	if (isWin8OrGreater())
	{
...
[[maybe_unused]] QMargins Utilities::getWindowNativeFrameMargins(const QWindow *window)
{
	Q_ASSERT(window);
	if (!window)
		return {};
...
[[maybe_unused]] QColor Utilities::getNativeWindowFrameColor(const bool isActive)
{
	if (!isActive)
		return Qt::darkGray;
	if (!isWin10OrGreater())
...
[[maybe_unused]] void Utilities::displaySystemMenu(const QWindow *window, const QPoint &pos)
{
	Q_ASSERT(window);
	if (!window)
		return;
