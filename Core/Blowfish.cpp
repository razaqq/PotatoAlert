...
void Blowfish::ReverseByteOrder(uint32_t& x)
{
	const uint32_t r = x;
	x = (r & 0xFF000000) >> 24;
	x |= (r & 0x00FF0000) >> 8;
...
void Blowfish::InitKey(std::span<const std::byte> key)
{
	...
	{
		uint32_t keyData = 0x00000000;
		for (uint32_t k = 0; k < 4; ++k)
		{
			keyData = (keyData << 8) | static_cast<uint32_t>(key[j++]);
			if (j >= key.size())
			{
				j = 0;
			}
		}
		m_pArray[i] = P[i] ^ keyData;
	}
...
bool Blowfish::Encrypt(std::span<const std::byte> src, std::span<std::byte> dst) const
{
	...
	for (size_t i = 0; i < dst.size() / BlockSize(); i++)
	{
		const size_t offset = i * BlockSize();
		uint32_t block[2];
		memcpy(block, src.data() + offset, sizeof(block));

		ReverseByteOrder(block[0]);
		ReverseByteOrder(block[1]);
		EncryptBlock(&block[0], &block[1]);
		ReverseByteOrder(block[0]);
		ReverseByteOrder(block[1]);

		memcpy(dst.data() + offset, block, sizeof(block));
	}
...
bool Blowfish::Decrypt(std::span<const std::byte> src, std::span<std::byte> dst) const
{
	...
	for (size_t i = 0; i < src.size() / BlockSize(); i++)
	{
		const size_t offset = i * BlockSize();
		uint32_t block[2];
		memcpy(block, src.data() + offset, sizeof(block));
