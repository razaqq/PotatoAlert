...
TEST_CASE( "GameTest_CheckPathTest" )
{
    ...
    folder = Game::checkPath(svcwdDir.string());
...
TEST_CASE( "GameTest_GetResFolderPathTest" )
{
    ...
    REQUIRE(status.resFolderPath == (nsnvDir / "bin" / "2666186").string() );

    status.gamePath = snvexeDir.string();
    status.steamVersion = true;
    REQUIRE( Game::getResFolderPath(status) );
    REQUIRE( status.folderVersion == "1427460" );
    REQUIRE(status.resFolderPath == (snvexeDir / "bin" / "1427460").string() );
}
...
TEST_CASE( "GameTest_ReadEngineConfigTest" )
{
    ...
    REQUIRE( Game::getResFolderPath(status) );
    REQUIRE( Game::readEngineConfig(status, "res") );
